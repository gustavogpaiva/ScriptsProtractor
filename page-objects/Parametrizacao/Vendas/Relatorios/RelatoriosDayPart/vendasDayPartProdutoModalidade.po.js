...
class vendasDayPartProdMod {
    ...
    selecionarUnidade(...unidade) {
        ...
        h.getIdGrid().then(function(idGrid){
            z.widget.grid.rowExists(columnName, unidade[0], idGrid).then(function(existeUnidade){
                if(existeUnidade)
                    z.widget.grid.click(columnName, unidade[0], idGrid);
            });
    ...
    selecionarProdInicial(produto) {
        z.field.fieldFunctions.click('NMPRODUTOI');
        browser.sleep(3000);
        $$("div.floating-card-input > input").get(1).click();
        $$("div.floating-card-input > input").get(1).sendKeys(produto);
        browser.sleep(3000);
        z.widget.grid.clickColumn('9009',0,0);
    };

    selecionarProdFinal(produto) {
...
module.exports = new vendasDayPartProdMod();