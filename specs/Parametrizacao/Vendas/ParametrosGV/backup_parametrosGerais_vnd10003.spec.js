...
describe('Testes da tela Parâmetros Gerais', function(){
	...
	it('Parametrização da aba Banco', function(){
		...
		paramGerais.editarParametros('Banco');
	...
	it('Parametrização da aba Cliente', function(){
		...
		paramGerais.editarParametros('Cliente');
	...
	it('Parametrização da aba Cliente - Tabela de preços', function(){
		...
		paramGerais.editarParametros('Tabelas de Preço');
	...
	it('Parametrização da aba Pagamento Online - Unidade', function(){
		...
		paramGerais.editarParametros('Unidade');
	...
	/*executa os testes de parametrização da aba Tipo de consumidor por Unidade*/
	xit('Editar Tipo de Consumidor por Unidade', function(){
		h.navegar('Tipo de Consumidor');
		h.navegar('Tipo de Consumidor por Unidade');
		paramGerais.filtrarUnidade(j.getValor('filial'));
		expect(paramGerais.selecionarTipoConsumidor(j.getValor('tipoConsumidor'))).toBe(true);
		...
		/*paramGerais.editarTipoConsumidorUnidade(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'S','1','S','2','S','3','S','4','S','5','S','6','S','7','S','8','S','9');
		expect(h.aguardaMensagem()).toBe('Operação realizada com sucesso.');
		h.fechaTela();
		//testa a parametrização de todas posições desmarcadas
		paramGerais.editarTipoConsumidorUnidade(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'N','1','N','2','N','3','N','4','N','1','N','6','N','7','N','8','N','9');
		expect(h.aguardaMensagem()).toBe('Operação realizada com sucesso.');
		h.fechaTela();
		//testa a parametrização de posições repetidas
		paramGerais.editarTipoConsumidorUnidade(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'S','1','S','2','S','3','S','4','S','1','S','6','S','7','S','8','S','9');
		expect(h.alertaDeErro()).toBe('Não é permitido repetir posições!');*/
	});
	...
	it('Bônus Carga de Crédito', function(){
		...
		paramGerais.bonusCargaCredito(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'S','12','2','12','Mensal','5',j.getValor('tipoRecebimento'));
		expect(h.aguardaMensagem()).toBe('Operação realizada com sucesso.');
		h.fechaTela();
		//testa a parametrização desmarcando o campo de utilização do bônus e limpando todos os campos
		paramGerais.bonusCargaCredito(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'N','','','','','','');
		expect(h.aguardaMensagem()).toBe('Operação realizada com sucesso.');
		h.fechaTela();
		//testa a parametrização deixando os campos obrigatórios em branco
		paramGerais.bonusCargaCredito(j.getValor('cdfilial'),j.getValor('tipoConsumidor'),'S','','','','','','');
		expect(h.campoObrigatorio()).toBe(true);
	});
	//executa o teste de exclusão de um tipo de consumidor por unidade
	it('Excluir Tipo de Consumidor por Unidade', function(){
		paramGerais.excluiTipoConsumidorUnidade(j.getValor('cdfilial'), j.getValor('tipoConsumidor'));
		expect(h.aguardaMensagem()).toBe('Excluído com sucesso!');
	});
	//executa o teste de parametrização da aba integrações
	it('Parametrização da aba Integrações', function(){
		//testa a parametrização da subaba operador
		paramGerais.operador(j.getValor('operador'), j.getValor('codigoExterno'));
		expect(h.aguardaMensagem()).toBe('Registros Salvos com Sucesso');
		h.fechaTela();
		//testa a parametrização da subaba imposto
		paramGerais.imposto(j.getValor('imposto'), j.getValor('codigoExterno'));
		expect(h.aguardaMensagem()).toBe('Registros Salvos com Sucesso');
		h.fechaTela();
		//testa a parametrização da subaba tipoRecebimento
		paramGerais.tipoRecebimento(j.getValor('tipoRecebimento'), j.getValor('codigoExterno'));
		expect(h.aguardaMensagem()).toBe('Registros Salvos com Sucesso');
	});*/
});